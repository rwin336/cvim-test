...
class CloudSanityManagementSpecification extends CloudSanityBaseSpecification {
    ...
    def "Test cloud-sanity create management test"() {
        given: "A valid Mercury REST API Client"
        assert client != null

        when: "The cloud-sanity create command is POSTed for a management test"
        def resp = client.post(path: 'cloudsanity/create',
                requestContentType: ContentType.JSON,
                headers: ['Content-Type': "application/json; charset=UTF-8"],
                body: ['cloudsanity_request':
                               ['command': 'create', 'action': 'test', 'test_name': 'management', 'uuid': '']])

        then: "A management test is created"
    ...
    def "Test cloud-sanity management test generates results"() {
        given: "A request for a cloud-sanity management test"
        assert client != null
        def resp = client.post(path: 'cloudsanity/create',
                requestContentType: ContentType.JSON,
                headers: ['Content-Type': "application/json; charset=UTF-8"],
        ...
        assert test_detail.message.message == "[PASSED] Cloud Sanity Management Checks Passed"

        and: "The results for management tests can be verified"
    ...
    def "Test create management test is blocked when another cloud-sanity test running"() {
        ...
        when: "A new request for cloud-sanity management test is create"
    ...
    def "Test delete is blocked util previous management test is complete"() {
        given: "A cloud-sanity test already running"
        assert client != null
        def resp = client.post(path: 'cloudsanity/create',
                requestContentType: ContentType.JSON,
        ...
        and: "The first management test completes successfully"
        waitForTest(test_uuid)
